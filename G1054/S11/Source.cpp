...
//to do: de gestionat centralizat salarizarea din ASE
int main()
{
	...
	Angajat listaA[10];
	listaA[0] = a;
	listaA[1] = p2;//angajat fost profesor; DE CE? pentru ca apel op= + upcast
	float totalSalarii = 0;
	for (int i = 0; i < 2; i++)
		totalSalarii += listaA[i].calculSalariu();
	cout << "\nSalarii totale: " << totalSalarii;

	Angajat* listaPointerA[10];
	listaPointerA[0] = &a;
	listaPointerA[1] = &p2;//pointer spre un profesor
	float totalSalarii2 = 0;
	for (int i = 0; i < 2; i++)
		totalSalarii2 += listaPointerA[i]->calculSalariu();
	cout << "\nSalarii totale: " << totalSalarii2;
	cout << "\n**************";
	return 0;
}